...
%% 载荷输入
Disp_Tip_Z_0 = 20*10^-3;   % 悬臂梁末端施加的初始位移
...
fprintf('end of %d loops\n', NUM_Step)

%% 结果输出

perturbation_tip_txt = fopen('perturbation_tip.txt', 'wt');

Perturbation_Tip_Z = Displacement(:,6*NUM_Cell_Node-3)*1000;   % 末端的位移响应【mm】

fprintf(perturbation_tip_txt, '%8.8f\n', Perturbation_Tip_Z); % 输出格式为浮点数,占用8位,保留8位小数
